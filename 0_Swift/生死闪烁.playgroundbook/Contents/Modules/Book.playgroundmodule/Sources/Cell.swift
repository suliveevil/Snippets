...
public enum State: Int {
    ...
    var description: String {
        switch self {
        case .alive:
            return NSLocalizedString("Alive Cell", comment:"a cell that is 'alive' in the simulation")
        case .dead:
            return NSLocalizedString("Dead Cell", comment:"A cell that is 'dead' in the simulation")
        case .idle:
            return NSLocalizedString("Idle Cell", comment:"a cell that is 'idle' in the simulation")
        }
    }
}

/// A cell in the simulation that can be in one of three states (alive, dead, or idle).
/// - localizationKey: Cell
public class Cell {
    ...
    /// - localizationKey: Cell.state
    public var state: State {
        didSet {
            if state != priorState {
                updateDisplay()
                spriteNode?.accessibilityLabel = state.description
            }
        }
    }
    ...
    /// - localizationKey: Cell.numberOfAliveNeighbors
    ...
    /// - localizationKey: Cell.numberOfDeadNeighbors
    ...
    /// - localizationKey: Cell.numberOfIdleNeighbors
    ...
    /// - localizationKey: Cell.stateNeighborCounts
    ...
    /// - localizationKey: Cell.forceState(_:)
    ...
    /**
     ...
     number of neighbors.
      - localizationKey: Cell.updatePriorState()
    */
